...
def start_host_game() -> Dict:
    ...
    client = _get_client()
  prompt = """스무고개 정답을 하나 정하고, '모호한 힌트' 한 줄을 만들어 주세요.
...
## 힌트 작성 규칙 (매우 중요)
힌트는 사용자가 게임을 시작할 방향만 흐릿하게 잡아주는 역할입니다.
...
3. 대신 **일상에서 자주 쓰는 친근한 단어**로, 사용자가 그 대상을 만나는 **상황·느낌·관계**를 묘사하세요.
   - "식탁에서", "아침마다", "집에 있으면", "뉴스에서", "사진으로", "운동장에서" 같은 일상 표현을 활용하세요.
...
{"answer": "정답 명사 하나", "hint": "위 규칙을 지킨 친근한 한 줄 힌트"}"""
